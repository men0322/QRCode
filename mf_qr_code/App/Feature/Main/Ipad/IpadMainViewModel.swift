...
class IpadMainViewModel: BaseViewModel, ViewModelType {
    let useCase: UseCase
    ...
    struct UseCase {
        let ipadMainUseCase: IpadMainUseCase
    ...
    func transform(_ input: Input) -> Output {
        let calculatorMode = input.loadTrigger
            .map { _ in self.useCase.ipadMainUseCase.inputNumberMode() }
